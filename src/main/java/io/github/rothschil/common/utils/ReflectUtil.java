...
public class ReflectUtil {
    ...
    public static <E> Specification<E> createSpecification(Map<String, String> objConditions, Class<E> clazz, List<String> excludeLikeAttr){
        return createSpecification(objConditions,clazz,excludeLikeAttr,null);
    }
    /**
     * 生成全属性条件查询通用Specification
     *
    ...
    public static <E> Specification<E> createSpecification(Map<String, String> objConditions, Class<E> clazz, List<String> excludeLikeAttr, Map joinField) {
        return (root, query, cb) -> {
            ...
            //未删除的数据
            try {
                clazz.getDeclaredField(Constant.VALID);
                if (!StringUtils.isEmpty(objConditions.get(Constant.VALID))) {
                    predicates.add(cb.equal(root.get(Constant.VALID), Integer.valueOf(objConditions.get(Constant.VALID))));
                } else {
            ...
            for (Field field : declaredFields) {
                ...
                if (!StringUtils.isEmpty(condition)) {
                    ...
                    try {
                        ...
                        throw new CommonException(Status.PARAMS_IS_INVALID);
                    }
    ...
    public static <E> Specification<E> createOneSpecification(String attr, String condition) {
        return (root, query, cb) -> {
            ...
            //未删除的数据
            try {
                if (Constant.VALID.equals(attr)) {
                    predicates.add(cb.equal(root.get(Constant.VALID), condition));
                } else {
                    predicates.add(cb.equal(root.get(Constant.VALID), 1));
                }
            } catch (Exception e) {
            ...
            //外键关联查询
            if (attr.contains(Constant.POINT)) {
                //递归解析真实的path
