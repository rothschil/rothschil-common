...
public class ExcelWriter {
     ...
     * @return 写入数据后的工作簿对象
     */
    public static Workbook exportData(List<IntfBo> dataList){
        // 生成xlsx的Excel
        ...
        //构建每行的数据内容
        int rowNum = 1;
        for (Iterator<IntfBo> it = dataList.iterator(); it.hasNext(); ) {
            IntfBo data = it.next();
            if (data == null) {
                continue;
            }
     ...
     * @return
     */
    private static void convertDataToRow(IntfBo data, Row row){
        int cellNum = 0;
        Cell cell;
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getInterfaceName() ? "" : data.getInterfaceName());
        cell = row.createCell(cellNum++);
